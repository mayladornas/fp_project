...
def newGameInfo(fileName):
    try:
        ...
        bottlesToFill = data['bottlesToFill']
        bottles_info = data['bottles_info']

    except FileNotFoundError:
        print(f"File not found: {fileName}")
        # Create a new game file with default values
        default_info = {
            'expertise': random.randint(MAX_EXPERT, LESS_EXPERT),
            'bottles_info': buildGameBottles(expertise),
            'bottlesToFill': NR_BOTTLES - expertise,
            'nrErrors': 0
        }
        with open(fileName, 'w') as new_file:
            json.dump(default_info, new_file)
        return default_info

    except json.JSONDecodeError as e:
        print(f"Warning: {fileName} {str(e)}. Starting a new game with default values.")
        # Create a new game file with default values
        default_info = {
            'expertise': random.randint(MAX_EXPERT, LESS_EXPERT),
            'bottles_info': buildGameBottles(expertise),
